...
public sealed class VoicevoxService : McpServiceBase, IGenerateVoiceService, IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            McpLogger?.Debug($"{LogPrefix} SaveTextFileAsync: テキストファイル保存なし (Mode: None)");
            return;
        }
        ...
        if (saveMode == TextFileSaveMode.TextOnly)
        {
            ...
            McpLogger?.Debug($"{LogPrefix} SaveTextFileAsync: テキストファイル保存中: {textFilePath}");

            await File.WriteAllTextAsync(textFilePath, text, System.Text.Encoding.UTF8, cancellationToken);

            McpLogger?.Debug($"{LogPrefix} SaveTextFileAsync: テキストファイル保存完了");
        }
        else if (saveMode == TextFileSaveMode.WithMetadata)
        {
            var jsonFilePath = Path.Combine(outputDir, $"{baseFileName}.json");
            McpLogger?.Debug($"{LogPrefix} SaveTextFileAsync: メタデータファイル保存中: {jsonFilePath}");
            ...
            McpLogger?.Debug($"{LogPrefix} SaveTextFileAsync: メタデータファイル保存完了");
        }
    }
}
